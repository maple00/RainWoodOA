...
public final class StaffAccountFragment extends BaseFragment implements IStaffCallbacks {
  ...
  /*  @ViewInject(R.id.tl_staff_account_type)
    ...
    protected void initListener() {
        mAccountAdapter.setItemAccount(new StaffAccountAdapter.OnClickItemAccount() {
            @Override
            public void onClickAccount(StaffAccount account) {
                // 查看会计账户详情
                // toast("查看会计账户详情");
                startActivity(getNewIntent(getContext(), PaymentDetailActivity.class, "收支详情"));
            }
        });
    }
    ...
    protected void loadData() {
        // 请求数据
        mStaffPresenter.requestAllAccountData();
    }
