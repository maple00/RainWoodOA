...
public class InnerPainter implements CalendarPainter {
    ...
    private Map<String, List<LocalDate>> mDayList;
    ...
    public InnerPainter(Context context, ICalendar calendar) {
        ...
        for (Map.Entry<String, List<LocalDate>> entry : mDayList.entrySet()) {
            ...
            for (int i = 0; i < holidayList.size(); i++) {
                entry.getValue().add(new LocalDate(holidayList.get(i)));
            }
            List<String> workdayList = CalendarUtil.getWorkdayList();
            for (int i = 0; i < workdayList.size(); i++) {
                entry.getValue().add(new LocalDate(workdayList.get(i)));
            }
        }
    }
    ...
    public void onDrawToday(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> checkedDateList) {
        if (checkedDateList.contains(localDate)) {
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.todayCheckedHoliday, mAttrs.todayCheckedWorkday, mAttrs.todayCheckedHolidayTextColor, mAttrs.todayCheckedWorkdayTextColor, noAlphaColor);
        } else {
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.todayUnCheckedHoliday, mAttrs.todayUnCheckedWorkday, mAttrs.todayUnCheckedHolidayTextColor, mAttrs.todayUnCheckedWorkdayTextColor, noAlphaColor);
        }
    ...
    public void onDrawCurrentMonthOrWeek(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> checkedDateList) {
        if (checkedDateList.contains(localDate)) {
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.defaultCheckedHoliday, mAttrs.defaultCheckedWorkday, mAttrs.defaultCheckedHolidayTextColor, mAttrs.defaultCheckedWorkdayTextColor, noAlphaColor);
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.defaultUnCheckedHoliday, mAttrs.defaultUnCheckedWorkday, mAttrs.defaultUnCheckedHolidayTextColor, mAttrs.defaultUnCheckedWorkdayTextColor, noAlphaColor);
        }
        drawStretchText(canvas, rectF, noAlphaColor, localDate);
    ...
    public void onDrawLastOrNextMonth(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> checkedDateList) {
        if (checkedDateList.contains(localDate)) {
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.defaultCheckedHoliday, mAttrs.defaultCheckedWorkday, mAttrs.defaultCheckedHolidayTextColor, mAttrs.defaultCheckedWorkdayTextColor, mAttrs.lastNextMothAlphaColor);
        } else {
            ...
            drawTagDay(canvas, rectF, localDate, mAttrs.defaultUnCheckedHoliday, mAttrs.defaultUnCheckedWorkday, mAttrs.defaultUnCheckedHolidayTextColor, mAttrs.defaultUnCheckedWorkdayTextColor, mAttrs.lastNextMothAlphaColor);
        }
    ...
    public void onDrawDisableDate(Canvas canvas, RectF rectF, LocalDate localDate) {
        ...
        drawTagDay(canvas, rectF, localDate, mAttrs.defaultUnCheckedHoliday, mAttrs.defaultUnCheckedWorkday, mAttrs.defaultUnCheckedHolidayTextColor, mAttrs.defaultUnCheckedWorkdayTextColor, mAttrs.disabledAlphaColor);
    ...
    //绘制节假日
    private void drawTagDay(Canvas canvas, RectF rectF, LocalDate localDate, Drawable holidayDrawable, Drawable workdayDrawable, int holidayTextColor, int workdayTextColor, int alphaColor) {
    ...
    public void setTagDayCopy(Map<String, List<String>> allDayList, int minShow) {
        ...
        for (String key : allDayList.keySet()) {
            ...
            if (mDayList.get(key) != null) {
                mDayList.get(key).clear();
            }
            // 遍历
            List<LocalDate> dayLocalList = new ArrayList<>();
            for (int i = 0; i < allDayList.get(key).size(); i++) {
                LocalDate holidayLocalDate;
                try {
                    holidayLocalDate = new LocalDate(allDayList.get(key).get(i));
                } catch (Exception e) {
